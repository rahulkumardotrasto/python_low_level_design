...
class Column:
    ...
    def validate(self):
        ...
        for constraint in self.constraints:
            constraint.id_ddl and constraint.validate()

    def validate_value(self, value):
        self.field_type.validate_value(value)
        for constraint in self.constraints:
            constraint.is_dml and constraint.validate_value(value)
...
def main():
    ...
    new_columns = [
        Column('id', fields.IntegerField(), [
               constraints.NotNullConstraint(), constraints.PrimaryKey()]),
        Column('name', fields.StringField(max_length=10),
               [constraints.NotNullConstraint()]),
        Column('age', fields.IntegerField(min_value=0, max_value=100), [
               constraints.NotNullConstraint()]),
    ]
    new_table = Table('student', columns=new_columns)
    new_table.add_record({'id': 1, 'name':  'rahul', 'age':  10})
    new_table.print_record(new_table.get_record(
        {'id': 1, 'name':  'rahul'}))
